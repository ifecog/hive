...
Two agents built with the \[Aden Hive Framework](https://github.com/aden-hive/hive) using Claude (Haiku 4.5) as the LLM and Brave Search for web retrieval.
...
\### 1. Research Agent X (`exports/research\_agent\_x/`)
...
hive run exports/research\_agent\_x --input '{"topic": "AI agents in healthcare 2025"}'
...
\### 2. Medication Checker (`exports/medication\_checker/`)
...
hive run exports/medication\_checker --input '{"medications": \["Lisinopril", "Ibuprofen", "Aspirin"]}'
...
After running `/hive-credentials`, opening a new terminal session caused a `CredentialDecryptionError` for the Brave Search key. The fix was running `source \~/.bashrc` to load the `HIVE\_CREDENTIAL\_KEY` environment variable. This is a subtle gotcha that could frustrate new users — a simple check at startup with a helpful error message would go a long way.
